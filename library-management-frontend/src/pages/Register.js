...
const Register = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      setToken(loginRes.data.token);
      setUser({ username, role: loginRes.data.user.role });
      localStorage.setItem('token', loginRes.data.token);
  ...
  return (
    <motion.div className="register-form-container">
      <form onSubmit={handleSubmit} className="register-form-box">
        <h2 className="register-title">Register</h2>
        <input type="text" placeholder="Username" value={username} onChange={e => setUsername(e.target.value)} required className="register-input" />
